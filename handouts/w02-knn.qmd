...
Last week we separated four research goals and left prediction as a promise. This week we keep it. The method we start with, $K$-nearest neighbors, was chosen for one reason above all others: it simple and straightforward. There is no probability theory to follow, no optimization procedure, and no coefficients to interpret. Everything the algorithm does can be seen in a picture, which means that when it goes wrong, you can see that too.
...
There is code in this handout, and you should run it rather than read it. The workshop part of the class is dedicated to this.
...
The most familiar strategy is to assume a shape for that relationship and then estimate the parameters of the assumed shape. Ordinary least squares assumes a line. Logit and probit assume particular shapes for binary outcomes. These are **parametric** models, and the assumption about the form of the function is doing a great deal of work.

Assuming a shape provides several benefits. You get coefficients you can interpret substantively, standard errors, a compact summary you can put in a table, and a direct connection between a theoretical claim and a quantity you can estimate. Most of what you have done in your quantitative methods courses in the past relies on this.
...
Last week's main example was the association between economic development and political development, and we stopped at the point where the correlation was clear and the direction of the arrow was not. Set the arrow aside for the moment. We are asking a prediction question now, and prediction does not require us to settle the direction of association.

We will use national development indicators for 199 countries, which comes with the `carData` package.
...
options(scipen = 999)   # print numbers in full rather than as scientific format of 1e+05
...
# Express per capita GDP in thousands of dollars
countries$ppgdp <- countries$ppgdp / 1000

nrow(countries)
head(countries)
```

`ppgdp` is *per capita* gross domestic product and `infantMortality` is deaths per thousand live births.

Two small things in that chunk. `options(scipen = 999)` tells R to print numbers in full rather than in scientific notation, so that a hundred thousand appears as `100000` and not as `1e+05`. And we divide per capita GDP by a thousand, so it is measured in thousands of dollars. That second step is cosmetic, since dividing every value by the same constant cannot change which countries are nearest to which, but it keeps every axis in this handout readable.
...
The shape of the association between these two variables is the main point here. Infant mortality falls very steeply across the poorest range and then flattens out almost completely. The difference between a country at \$500 and a country at \$2,000 is enormous. The difference between \$30,000 and \$40,000 is close to nothing.

Now put a straight line, via a linear regression, through it.
...
The line is not a small approximation to the true association here. It is systematically wrong in a patterned way: too low for the poorest countries, too high through the middle, and it eventually predicts negative infant mortality, which is not a quantity that exists.

You could repair this without leaving the parametric framework, and you probably did it before. Taking the logarithm of per capita GDP would straighten most of the curve, and that transformation has a defensible substantive reading: what matters for infant mortality is proportional differences in income, not absolute ones. That is a real modeling decision and a good one. Of course, this is not always the solution.
...
Nothing so far told us to use ten for $k$ . Try several values.
...
kvals  <- c(3, 10, 50, 150, 193)
kcols  <- c("darkgreen", "maroon", "navy", "cyan", "pink")
...
At $k=3$ the curve is jagged. It follows every local fluctuation, including fluctuations produced by the fact that we happen to have these 199 countries rather than some other set. At $k=150$ the curve is nearly flat, close to the overall average, and it has erased a difference between rich and poor countries that is obviously real. At $k=193$, it is completely flat. Why?

Both extreme levels of $k$ are informative, and they are opposite failures.
...
What we want is $f(.)$. The term $\varepsilon$ is whatever is left over: measurement error, idiosyncrasy, the particular history of the particular countries, COVID, and so forth in this dataset. It is not a stable feature of the world, and a model that has learned it has learned something that will not recur.
...
To choose between these we need to stop evaluating the model on the data it was built from. Split the data in two: fit on one part, evaluate on the other.
...
`set.seed()` fixes the random draw so that you and I get the same split. Use it every time you do randomization. Without it your results change on every run and neither you nor anyone else can reproduce them.
...
for (i in seq_along(kseq)) {
  f_test <- kknn(infantMortality ~ ppgdp, train=Train, test=Test,
                 k = kseq[i], kernel = "rectangular")
  err_test[i] <- rmse(Test$infantMortality, f_test$fitted.values)

  f_train <- kknn(infantMortality ~ ppgdp, train=Train, test=Test,
                  k = kseq[i], kernel = "rectangular")
...
Read the two curves against each other, because the comparison is a fundamental lesson in Machine Learning.
...
One caution before you go looking for the best $k$. Change the seed and rerun the code. The U shape will still be there; the exact location of the minimum will move. The shape is a property of the trade off. The precise minimum is a property of one arbitrary split. Do not report "the optimal $k$ is 11" as though you had discovered something about infant mortality. It is not a globally optimal $k$. We will fix this properly in a moment.
...
We now have the picture of a well-known trade-off in Machine Learning literature. The vocabulary for it is worth learning, because it is how everyone else in this literature talks, and because the two failures it names are genuinely different failures.
...
Now back to the problem we identified. We need to pick the 'best' $k$, and we know that there is a trade-off that needs to be handled. We chose $k$ using one train and test split, and the choice moved when the seed moved. Two complaints about the single split, both fair:
...
2.  We set aside thirty percent of the countries in our sample and never trained on them. With 199 cases that is a real loss of information.
...
`caret` is the standard R package for fitting and comparing predictive models through a single interface. It is worth installing now, because we use it again in this course.
...
yhat <- predict(cv_caret, newdata = Test)               # a held-out set
rmse(Test$infantMortality, yhat)
```
...
Our features are measured on very different scales. Per capita GDP runs from well under one up to about a hundred, in thousands of dollars. Percent urban runs from zero to a hundred. Fertility runs from about one to about seven, and female life expectancy from the high forties to the high eighties.

```{r}
summary(countries[, c("ppgdp", "pctUrban", "fertility", "lifeExpF")])
```

Put those into the distance formula unchanged and the two wide-ranging features decide everything. A difference of fifty between two countries on per capita GDP contributes two and a half thousand to the sum under the square root. The entire possible range of fertility, from one child per woman to seven, contributes about thirty-five. Fertility is not being weighted lightly here. For practical purposes it is not in the model at all, and neither is anything else measured on a small scale.
...
plot(countries$ppgdp, countries$infantMortality, col = "maroon", cex = 0.7,
     xlab = "Per capita GDP (thousands of US$)", ylab = "Infant mortality",
...
target_raw <- c(median(countries$ppgdp),   median(countries$fertility))
target_s   <- c(median(countries_s$ppgdp), median(countries_s$fertility))

# Euclidean distance, exactly the formula above, computed by hand
dist_raw <- sqrt((countries$ppgdp     - target_raw[1])^2 +
                 (countries$fertility - target_raw[2])^2)

dist_s   <- sqrt((countries_s$ppgdp     - target_s[1])^2 +
                 (countries_s$fertility - target_s[2])^2)
...
plot(countries$ppgdp, countries$fertility, col = "gray55", pch = 19, cex = 0.7,
     xlab = "Per capita GDP (thousands of US$)", ylab = "Fertility (births per woman)",
     main = "Raw units, k = 25", font.main = 1)
points(countries$ppgdp[nb_raw], countries$fertility[nb_raw], col = "red", cex = 1.6)
points(target_raw[1], target_raw[2], col = "red", pch = 19, cex = 1.7)
abline(v = target_raw[1], h = target_raw[2], col = "darkgreen", lty = 2)

plot(countries_s$ppgdp, countries_s$fertility, col = "gray55", pch = 19, cex = 0.7,
     xlab = "Per capita GDP (rescaled)", ylab = "Fertility (rescaled)",
     main = "Rescaled, k = 25", font.main = 1)
points(countries_s$ppgdp[nb_s], countries_s$fertility[nb_s], col = "red", cex = 1.6)
...
On the left it is a narrow vertical band. The selected countries share almost exactly the target's income and are spread across nearly the whole range of fertility, from around one child per woman to well above the average. Per capita GDP spans about a hundred units on this scale and fertility spans about six, so a difference of a few units of income outweighs the entire fertility axis. The algorithm did not ignore fertility because anybody judged it unimportant. It ignored it because of the units it happens to be measured in.
