...
# Print numbers in full rather than as 1e+05. Affects plot axes too.
options(scipen = 999)
...
`ppgdp` is per capita gross domestic product in US dollars, and `infantMortality` is deaths per thousand live births.

The `options(scipen = 999)` line above is a small housekeeping habit worth adopting now. Per capita GDP runs into six figures, and R's default is to report large and small numbers in scientific notation, so an axis would be labeled `0e+00`, `2e+04`, `4e+04` rather than in dollars anyone can read. Setting `scipen` high tells R to prefer ordinary notation, in printed output and on plot axes alike. Put it at the top of any script whose numbers are meant for a reader rather than for you.
