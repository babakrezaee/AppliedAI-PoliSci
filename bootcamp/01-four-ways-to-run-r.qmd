# Four Ways to Run R Code {.unnumbered}
...
model <- glm(onset ~ log_pop + gdpen, data = fl_clean, family = "binomial")
summary(model)
```
As explained in the previous section, you can check the dimension of your datasets and summarize your variables as well:
...
```{{r}}
model <- glm(onset ~ gdpen + ethfrac, data = fl, family = "binomial")
summary(model)
```
...
ggplot(fl, aes(x = gdpen, y = fitted)) +
  geom_line()
```
